...
import { assertRedspotInvariant, RedspotError } from '../internal/core/errors';
...
const log = debug('redspot:core:tasks:compile');
    ...
    async (
      ...
      if (compilerPathResult === undefined) {
        if (desiredPlatform === CompilerPlatform.WASM) {
          ...
          // we can do
          throw new RedspotError(ERRORS.SOLC.CANT_GET_COMPILER, {
            version: solcVersion
          });
        }
...
subtask(TASK_COMPILE_SOLIDITY_LOG_COMPILATION_ERRORS)
  ...
  .setAction(async ({ output, quiet }: { output: any; quiet: boolean }) => {
    ...
    if (hasConsoleErrors) {
      console.error(
        chalk.red(
          `The console.log call you made isn’t supported. See https://redspot.org/console-log for the list of supported methods.`
        )
      );
      console.log();
    }
  ...
  .setAction(
    async ({ output, quiet }: { output: any; quiet: boolean }, { run }) => {
      ...
      if (hasCompilationErrors(output)) {
        throw new RedspotError(ERRORS.BUILTIN_TASKS.COMPILE_FAILURE);
      }
    }
  );
...
`;
      ...
      reasons += `Learn more about compiler configuration at https://redspot.org/config
`;
