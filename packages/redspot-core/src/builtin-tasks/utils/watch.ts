...
const log = debug('redspot:core:compilation-watcher');
  ...
  const addCompilationResult = async (buildInfo: string) => {
    try {
      ...
      await provider.request({
        method: 'redspot_addCompilationResult',
        params: [solcVersion, input, output]
      });
    } catch (error) {
      console.warn(
        chalk.yellow(
          'There was a problem adding the new compiler result. Run Redspot with --verbose to learn more.'
        )
      );

      log(
        "Last compilation result couldn't be added. Please report this to help us improve Redspot.\n",
        error
      );
