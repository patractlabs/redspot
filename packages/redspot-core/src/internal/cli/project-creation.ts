...
const SAMPLE_PROJECT_DEPENDENCIES: Dependencies = {
  ...
  chai: '^4.2.0',
  '@nomiclabs/redspot-ethers': '^2.0.0',
  ethers: '^5.0.0'
};
...
function printSuggestedCommands() {
  ...
  console.log(`  npx redspot accounts`);
  console.log(`  npx redspot compile`);
  console.log(`  npx redspot test`);
  console.log(`  npx redspot node`);
  console.log(`  node scripts/sample-script.js`);
  console.log(`  npx redspot help`);
}
...
async function writeEmptyRedspotConfig() {
  return fsExtra.writeFile(
    'redspot.config.js',
    `/**
 * @type import('redspot/config').RedspotUserConfig
 */
module.exports = {
...
export async function createProject() {
  ...
  if (await canInstallRecommendedDeps()) {
    ...
    } else if (installedExceptRedspot.length === 0) {
...
export async function confirmTelemetryConsent(): Promise<boolean | undefined> {
  ...
  const prompt = new enquirer.prompts.Confirm({
    ...
    initial: true,
    message:
      'Help us improve Redspot with anonymous crash reports & basic usage data?'
  });
