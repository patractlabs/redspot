...
You can learn how to use Redspot by reading the [Getting Started guide](../getting-started).`,
...
This is probably caused by a programming error in redspot or in one of the used plugins.
...
Please [report it](https://github.com/nomiclabs/redspot/issues/new) to help us improve Redspot.`,
    ...
    NON_LOCAL_INSTALLATION: {
      number: 12,
      message:
        'Trying to use a non-local installation of Redspot, which is not supported.\nPlease install Redspot locally using npm or Yarn, and try again.',
      title: 'Redspot is not installed or installed globally',
      description: `You tried to run Redspot from a global installation or not installing it at all. This is not supported.
...
Please install Redspot locally using npm or Yarn, and try again.`,
    ...
    TS_NODE_NOT_INSTALLED: {
      number: 13,
      message: `Your Redspot project uses typescript, but ts-node is not installed.

Please run: npm install --save-dev ts-node`,
      title: 'ts-node not installed',
      description: `You are running a Redspot project that uses typescript, but you haven't installed ts-node.
...
Please run this and try again: npm install --save-dev ts-node`,
    ...
    TYPESCRIPT_NOT_INSTALLED: {
      number: 14,
      message: `Your Redspot project uses typescript, but it's not installed.

Please run: npm install --save-dev typescript`,
      title: 'typescript not installed',
      description: `You are running a Redspot project that uses typescript, but it's not installed.
...
Read the [documentation](https://redspot.org/config/#networks-configuration) to learn how to define custom networks.`,
      shouldBeReported: false
    },
...
Read the [documentation](https://redspot.org/config/#hd-wallet-config) to learn how to define HD accounts correctly.`,
      shouldBeReported: false
    },
...
You must always use slashes (/) in Solidity imports.`,
    ...
    INVALID_IMPORT_PROTOCOL: {
      number: 406,
      message:
        "Invalid import %imported% from %from%. Redspot doesn't support imports via %protocol%.",
...
You can only import files thar are available locally or installed through npm.`,
    ...
    INVALID_IMPORT_ABSOLUTE_PATH: {
      number: 407,
      message:
        "Invalid import %imported% from %from%. Redspot doesn't support imports with absolute paths.",
...
This is not supported by Redspot.`,
      shouldBeReported: false
    },
...
Redspot's compiler is case sensitive to ensure projects are portable across different operating systems.`,
      shouldBeReported: false
    },
...
Please check your Internet connection.`,
    ...
    CANT_GET_COMPILER: {
      ...
      description: `Redspot couldn't obtain a valid solc compiler.
...
We recommend not using this kind of dependencies.`,
    ...
    JSONRPC_UNSUPPORTED_NETWORK: {
      number: 605,
      message:
        'Unsupported network for JSON-RPC server. Only redspot is currently supported.',
      title: 'Unsupported network for JSON-RPC server.',
      description: `JSON-RPC server can only be started when running the Redspot Network.
...
`,
      ...
      description: `There are multiple artifacts that match the given contract name, and Redspot doesn't know which one to use.
...
Redspot's artifact resolution is case sensitive to ensure projects are portable across different operating systems.`,
      shouldBeReported: true
    }
  },
  PLUGINS: {
...
Redspot plugin instead.`,
      title: 'Using a buidler plugin',
      description: `You are trying to use a Buidler plugin in Redspot. This is not supported.

Please use the equivalent Redspot plugin instead.`,
      shouldBeReported: false
    },
