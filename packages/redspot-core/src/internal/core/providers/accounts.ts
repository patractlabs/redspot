...
export class LocalAccountsProvider extends ProviderWrapperWithChainId {
  ...
  public async request(args: RequestArguments): Promise<unknown> {
    ...
    if (args.method === 'eth_sign') {
      ...
      if (address !== undefined) {
        if (data === undefined) {
          throw new RedspotError(ERRORS.NETWORK.ETHSIGN_MISSING_DATA_PARAM);
        }
  ...
  private _getPrivateKeyForAddress(address: string): Buffer {
    ...
    if (pk === undefined) {
      throw new RedspotError(ERRORS.NETWORK.NOT_LOCAL_ACCOUNT, {
        account: address
      });
    }
    ...
    if (
      ...
      if (tx !== undefined && tx.from === undefined) {
        ...
        if (senderAccount !== undefined) {
          ...
          throw new RedspotError(ERRORS.NETWORK.NO_REMOTE_ACCOUNT_AVAILABLE);
        }
      }
    }
