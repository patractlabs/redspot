...
import { BoundExperimentalRedspotNetworkMessageTraceHook } from '../../../../types';
...
export class EthModule {
  ...
  private async _callAction(
    ...
    if (trace !== undefined) {
      await this._runRedspotNetworkMessageTraceHooks(trace, true);
    }
  ...
  private async _sendTransactionAndReturnHash(tx: Transaction) {
    ...
    if (trace !== undefined) {
      await this._runRedspotNetworkMessageTraceHooks(trace, false);
    }
  ...
  private async _runRedspotNetworkMessageTraceHooks(
    trace: MessageTrace,
    isCall: boolean
  ) {
    for (const hook of this._experimentalRedspotNetworkMessageTraceHooks) {
