...
import { RedspotModule } from './modules/redspot';
...
const log = debug('redspot:core:redspot-network:provider');
...
export class RedspotNetworkProvider extends EventEmitter
  ...
  constructor(
    ...
    private readonly _experimentalRedspotNetworkMessageTraceHooks: BoundExperimentalRedspotNetworkMessageTraceHook[] = [],
  ...
  public async request(args: RequestArguments): Promise<unknown> {
    ...
    if (args.params !== undefined && !Array.isArray(args.params)) {
      throw new InvalidInputError(
        "Redspot Network doesn't support JSON-RPC params sent as an object"
      );
    }
  ...
  private async _sendWithLogging(
    ...
    try {
      ...
      if (err instanceof SolidityError) {
        this._logError(err);
      } else if (err instanceof RedspotNetworkProviderError) {
  ...
  private async _send(method: string, params: any[] = []): Promise<any> {
    ...
    if (method.startsWith('redspot_')) {
      return this._redspotModule!.processRequest(method, params);
    }
  ...
  private async _init() {
    ...
    const [common, node] = await RedspotNode.create(config);
  ...
  private async _makeTracingConfig(): Promise<TracingConfig | undefined> {
    if (this._artifacts !== undefined) {
      ...
      try {
        ...
        console.warn(
          chalk.yellow(
            'Stack traces engine could not be initialized. Run Redspot with --verbose to learn more.'
          )
        );

        log(
          "Solidity stack traces disabled: Failed to read solc's input and output files. Please report this to help us improve Redspot.\n",
          error
        );
      }
    }
  ...
  private _forwardNodeEvents(node: RedspotNode) {
    node.addListener('ethEvent', this._ethEventListener);
  }

  private _stopForwardingNodeEvents(node: RedspotNode) {
