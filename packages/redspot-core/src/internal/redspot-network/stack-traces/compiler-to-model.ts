  ...
  for (const bytecode of bytecodes) {
    ...
    for (const [signature, hexSelector] of Object.entries(methodIdentifiers)) {
      ...
      if (!fixedSelector) {
        ...
        throw new Error(
          `Failed to compute the selector one or more implementations of ${contract.name}#${functionName}. Redspot Network can automatically fix this problem if you don't use function overloading.`
        );
      }
    }
  }
