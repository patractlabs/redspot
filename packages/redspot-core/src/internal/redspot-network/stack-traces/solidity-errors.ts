...
export function encodeSolidityStackTrace(
  ...
  Error.prepareStackTrace = (error, stack) => {
    if (previousStack !== undefined) {
      ...
      // We remove Redspot Network related stack traces
      stack.splice(0, 3);
    }
...
function getMessageFromLastStackTraceEntry(
  ...
  switch (stackTraceEntry.type) {
    ...
    case StackTraceEntryType.OTHER_EXECUTION_ERROR:
      return `Transaction reverted for an unrecognized reason. Please report this to help us improve Redspot.`;
...
// Note: This error class MUST NOT extend RedspotNetworkProviderError, as libraries
