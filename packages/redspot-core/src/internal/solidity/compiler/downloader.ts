...
const log = debug('redspot:core:solidity:downloader');
...
export class CompilerDownloader {
  ...
  public async verifyCompiler(
    ...
    if (expectedKeccak256 !== compilerKeccak256) {
      ...
      throw new RedspotError(ERRORS.SOLC.INVALID_DOWNLOAD, {
  ...
  public async getDownloadedCompilerPath(
    ...
    try {
      ...
      if (RedspotError.isRedspotError(e)) {
        throw e;
      }
      console.warn(
        chalk.yellow(
  ...
  private async _getCompilerBuildByPlatform(
    ...
    if (compilerBuild === undefined) {
      throw new RedspotError(ERRORS.SOLC.INVALID_VERSION, { version });
    }
