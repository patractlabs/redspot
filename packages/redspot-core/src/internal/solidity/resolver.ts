...
export class Resolver {
  ...
  public async resolveImport(
    ...
    if (scheme !== undefined) {
      throw new RedspotError(ERRORS.RESOLVER.INVALID_IMPORT_PROTOCOL, {
    ...
    if (replaceBackslashes(imported) !== imported) {
      throw new RedspotError(ERRORS.RESOLVER.INVALID_IMPORT_BACKSLASH, {
        from: from.sourceName,
        imported
      });
    }
    ...
    if (isAbsolutePathSourceName(imported)) {
      throw new RedspotError(ERRORS.RESOLVER.INVALID_IMPORT_ABSOLUTE_PATH, {
        from: from.sourceName,
        imported
      });
    }
    ...
    try {
      ...
      if (
        RedspotError.isRedspotErrorType(
          error,
  ...
  private async _resolveLibrarySourceName(
    ...
    try {
      ...
      // if the project is using a dependency from redspot itself but it can't
      // be found, this means that a global installation is being used, so we
      // resolve the dependency relative to this file
      if (libraryName === 'redspot') {
        const redspotCoreDir = path.join(__dirname, '..', '..');
        packageJsonPath = path.join(redspotCoreDir, 'package.json');
      } else {
        throw new RedspotError(
          ERRORS.RESOLVER.LIBRARY_NOT_INSTALLED,
          {
  ...
  private async _relativeImportToSourceName(
    ...
    if (
      ...
      throw new RedspotError(ERRORS.RESOLVER.ILLEGAL_IMPORT, {
        from: from.sourceName,
        imported
      });
    }
  ...
  private async _validateSourceNameExistenceAndCasing(
    ...
    try {
      ...
      if (
        RedspotError.isRedspotErrorType(error, ERRORS.SOURCE_NAMES.WRONG_CASING)
      ) {
        throw new RedspotError(
          ERRORS.RESOLVER.WRONG_SOURCE_NAME_CASING,
          {
